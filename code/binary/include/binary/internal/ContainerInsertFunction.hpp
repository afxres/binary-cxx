...
struct ContainerInsertFunction<T> {
    ...
    static void Invoke(T& container, std::ranges::range_value_t<T>&& result) { container.emplace(std::move(result)); }
};
...
struct ContainerInsertFunction<T> {
    static constexpr bool IsEnable = true;
    static void Invoke(T& container, std::ranges::range_value_t<T>&& result) { container.emplace_back(std::move(result)); }
};
}
