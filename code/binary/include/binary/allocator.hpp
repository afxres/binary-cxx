...
public:
    allocator(abstract_buffer_ptr shared)
        : allocator(shared, length_max) {}
    ...
    length_t capacity() const noexcept {
        return bounds_;
    }

    length_t max_capacity() const noexcept {
        return limits_;
    }

    span as_span() const noexcept {
        return span(shared_, buffer_, offset_);
    }
