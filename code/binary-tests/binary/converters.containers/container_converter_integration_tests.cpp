...
void __test_container_converter_length(abstract_converter_ptr<_Ty> underlying_converter) {
    ...
    BOOST_TEST_MESSAGE(new_line << typeid(_Container).name() << new_line << typeid(_Ty).name() << new_line << "underlying converter size: "
                                << underlying_converter->length() << new_line << "container converter size : " << converter.length());
