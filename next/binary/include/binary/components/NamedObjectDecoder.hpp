...
public:
    NamedObjectDecoder(const std::vector<std::string>& names, const std::vector<std::vector<std::byte>>& headers);
    void Invoke(const std::span<const std::byte>& span, std::vector<std::tuple<bool, std::span<const std::byte>>>& slices);

private:
    [[noreturn]]
    void ExceptKeyFound(size_t index);
    [[noreturn]]
    void ExceptNotFound(size_t index);

    const std::vector<std::string>& names;
    std::map<std::string_view, size_t> indexes;
};
}
