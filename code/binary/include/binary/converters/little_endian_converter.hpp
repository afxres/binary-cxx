...
public:
    little_endian_converter()
        : abstract_converter<T>::abstract_converter(sizeof(T)) {}

    virtual void encode(allocator& allocator, const T& item) override {
        allocator::append<const T&>(
            allocator, sizeof(item), item,
            [](byte_ptr buffer, length_t length, const T& status) {
                endian::encode_little_endian(buffer, status);
            });
    }
