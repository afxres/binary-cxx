...
namespace mikodev::binary
{
    ...
    private:
        ...
        static void encode(byte_ptr buffer, number_t number, length_t length)
        {
            ...
            if (length == 1)
                endian::encode_big_endian<uint8_t>(buffer, static_cast<uint8_t>(number));
            else
                endian::encode_big_endian<uint32_t>(buffer, static_cast<uint32_t>(number) | 0x8000'0000);
        }
        ...
        static number_t decode(const_byte_ptr buffer, length_t& offset, length_t limits)
        {
            ...
            uint32_t header = static_cast<uint32_t>(endian::decode_big_endian<uint8_t>(source));
            offset += 1;
            if ((header & 0x80U) == 0)
            ...
            if (limits - offset < 3U)
                exceptions::throw_helper::throw_not_enough_bytes();
            uint32_t result = endian::decode_big_endian<uint32_t>(source);
            offset += 3;
