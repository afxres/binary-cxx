...
template <template <typename> typename TConverter, std::ranges::range TRange>
    requires std::derived_from<TConverter<TRange>, IConverter> && std::constructible_from<TConverter<TRange>, const std::shared_ptr<Converter<std::ranges::range_value_t<TRange>>>&>
...
template <template <typename> typename TConverter, template <typename...> typename TTuple, typename... TArguments>
    requires std::derived_from<TConverter<TTuple<TArguments...>>, IConverter> && std::constructible_from<TConverter<TTuple<TArguments...>>, const std::shared_ptr<Converter<std::remove_cv_t<TArguments>>>&...> && requires { std::tuple_size<TTuple<TArguments...>>::value; }
